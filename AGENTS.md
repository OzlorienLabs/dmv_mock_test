...
- **Leaderboard** (`src/lib/leaderboard/`): a **public, self-published** scoreboard — no server aggregation. Each signed-in (non-anonymous) user's score is `leaderboardScore(attempts)` = the count of **unique questions whose latest answer was correct** (dedup by question; a later wrong answer drops that question's point; never negative). The score is written to `leaderboard/{uid}` (**public-read**, owner-write) from the progress provider whenever attempts change — best-effort, skipped when unchanged (a local signature) or opted out. The board is public so **guests see it too** (a sign-in hook); the page shows the top 10 (`orderBy score desc`) to everyone, plus — only for a signed-in user — their own rank via a `getCountFromServer(score > mine)` aggregation. Opt-out is on by default; the preference lives on the owner-only `users/{uid}.leaderboardOptOut` and, when set, deletes the public entry. Scores are client-computed, so treat the board as social, not anti-cheat.
